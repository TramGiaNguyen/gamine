...
const getApiUrl = () => {
  ...
  // Fallback: Use the hostname from the current URL
  const hostname = window.location.hostname;
  return `http://${hostname}:8000/api`;
};

// Tạo instance axios với base URL từ biến môi trường hoặc IP máy host
...
export const fetchProductsByPromotion = async (promotionId) => {
  try {
    ...
    if (response.ok) {
      ...
      if (data.length === 0) {
        console.log(`No products returned from API, trying fallback method`);
        return await fetchProductsByPromotionFallback(promotionId);
      }
          ...
          try {
            ...
            if (completeProduct.ok) {
              ...
              return {
                ...
                discounted_price: product.discounted_price || (productData.price * (1 - (product.discount_percentage || 0) / 100))
              };
            }
          } catch (error) {
...
// Fallback function to find products with matching promotion when API fails
const fetchProductsByPromotionFallback = async (promotionId) => {
  try {
    ...
    // Lấy các danh mục được áp dụng khuyến mãi (nếu có)
    let promotionCategories = [];
    try {
      const categoryResponse = await fetch(`${API_URL}/promotions/${promotionId}/categories/`);
      if (categoryResponse.ok) {
        promotionCategories = await categoryResponse.json();
        console.log(`Found ${promotionCategories.length} categories for promotion ${promotionId}:`, 
          promotionCategories.map(c => c.category_id).join(', '));
      }
    } catch (err) {
      console.error(`Error fetching promotion categories:`, err);
    }
    
    // Extract category IDs from the promotion categories
    const promotionCategoryIds = promotionCategories.map(c => parseInt(c.category_id));
    
    // Get products with discounted prices
    const productsWithPromotions = await Promise.all(
      allProducts.map(async (product) => {
        try {
          // Check if product belongs to a category in the promotion
          const productCategoryId = parseInt(product.category_id);
          const isCategoryPromoted = promotionCategoryIds.includes(productCategoryId);
          
          // Check if product has a direct promotion
          const promotionInfo = await getProductDiscountedPrice(product);
          
          if (promotionInfo) {
            // Product has a direct promotion
            console.log(`Product ${product.product_id} (${product.name}) has direct discount: ${promotionInfo.discountPercentage}%`);
            return {
              ...product,
              has_promotion: true,
          ...
          } else if (isCategoryPromoted) {
            // Product's category has a promotion
            console.log(`Product ${product.product_id} (${product.name}) has category discount: category_id=${productCategoryId}`);
            const discount = targetPromotion.discount_percentage || 0;
    ...
    // Filter products that have a promotion and potentially match the promotion
    const filteredProducts = productsWithPromotions.filter(product => {
      // Basic check if product has any promotion
      const hasPromotion = product.has_promotion === true;
      
      // Check if discount matches the target promotion
      const exactDiscountMatch = targetPromotion.discount_percentage && 
                             parseInt(product.discount_percentage) === parseInt(targetPromotion.discount_percentage);
      
      // Check if product belongs to a promoted category
      const productCategoryId = parseInt(product.category_id);
      const isCategoryPromoted = promotionCategoryIds.includes(productCategoryId);
      
      const shouldInclude = hasPromotion && (exactDiscountMatch || isCategoryPromoted);
      
      // Log detailed decision process for debugging
      if (hasPromotion) {
        console.log(`Product ${product.product_id} (${product.name}): 
          - has_promotion: ${hasPromotion}
          - discount_match: ${exactDiscountMatch} (product: ${product.discount_percentage}%, promo: ${targetPromotion.discount_percentage}%)
          - category_promoted: ${isCategoryPromoted} (product category: ${productCategoryId})
    ...
    console.log(`Found ${filteredProducts.length} products with matching promotion criteria`);
    
    return filteredProducts;
  } catch (error) {
    console.error('Error in promotion fallback:', error);
    return [];
  }
};
