...
def dashboard_stats(request):
    try:
        ...
        # Thống kê thiết bị truy cập từ bảng UserActivityLog
        device_stats = []
        
        # Tối ưu truy vấn để chỉ phân loại thành Desktop và Mobile
        mobile_keywords = ['mobile', 'android', 'iphone', 'ipad', 'phone', 'tablet']
        
        # Tạo query để lọc thiết bị di động
        mobile_query = Q()
        for keyword in mobile_keywords:
            mobile_query |= Q(device__icontains=keyword)
        
        # Đếm số lượng thiết bị mobile
        mobile_count = UserActivityLog.objects.filter(
            device__isnull=False
        ).filter(mobile_query).count()
        
        # Đếm tổng số thiết bị có ghi nhận
        total_devices = UserActivityLog.objects.filter(device__isnull=False).count()
        
        # Tính số lượng thiết bị desktop (tổng số trừ đi số mobile)
        desktop_count = total_devices - mobile_count
        
        # Đảm bảo luôn có dữ liệu hiển thị
        if total_devices == 0:
            # Dữ liệu mẫu nếu không có logs
            device_stats = [
            ...
            device_stats = [
                {'device_type': 'Desktop', 'count': desktop_count},
                {'device_type': 'Mobile', 'count': mobile_count}
            ]
        
        # Thống kê bán hàng theo danh mục sản phẩm
        # Lấy tổng số lượng đã bán theo danh mục
        category_sales = []
        
        # Truy vấn số lượng sản phẩm đã bán theo danh mục
        category_data = Categories.objects.annotate(
            total_sold=Sum('products__sold_quantity')
        ).order_by('-total_sold')
        
        for category in category_data:
            # Bỏ qua các danh mục không có sản phẩm nào được bán
...
def promotion_products(request, promotion_id):
    try:
        ...
        # Trường hợp 1: Lấy các sản phẩm được khuyến mãi trực tiếp
        product_promotions = ProductPromotions.objects.filter(promotion=promotion, product__isnull=False)
        
        # Thêm thông tin sản phẩm được khuyến mãi trực tiếp
        for pp in product_promotions:
            product = pp.product
            
            # Tính giá sau khi giảm giá
            original_price = float(product.price)
            discount_percentage = promotion.discount_percentage or 0
        ...
        # Trường hợp 2: Lấy các sản phẩm thuộc danh mục được khuyến mãi
        category_promotions = ProductPromotions.objects.filter(promotion=promotion, category__isnull=False)
        
        # Thu thập tất cả các ID danh mục được khuyến mãi
        category_ids = [cp.category.category_id for cp in category_promotions]
        
        if category_ids:
            # Nếu có bất kỳ danh mục nào, lấy tất cả sản phẩm thuộc các danh mục đó
            category_products = Products.objects.filter(category_id__in=category_ids)
            
            # Loại bỏ sản phẩm đã được thêm ở trên (để tránh trùng lặp)
            added_product_ids = [p['product_id'] for p in product_data]
            category_products = category_products.exclude(product_id__in=added_product_ids)
            
            # Thêm thông tin sản phẩm thuộc danh mục được khuyến mãi
        ...
        print(f"Found {len(product_data)} products for promotion ID {promotion_id} (direct + by category)")
        
        return Response(product_data)
    except Promotions.DoesNotExist:
        return Response({"error": "Khuyến mãi không tồn tại"}, status=status.HTTP_404_NOT_FOUND)
    except Exception as e:
        print(f"Error in promotion_products: {str(e)}")
        return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
